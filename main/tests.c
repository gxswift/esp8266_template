...
esp_err_t index_handler(httpd_req_t *req)
{
    ...
    spi_flash_read(500 * 4096, buf, buf_len-1);
...
esp_err_t heart_get_handler(httpd_req_t *req)
{
    char*  buf;
    size_t buf_len;
    ESP_LOGI(TAG, "Free Stack for server task: '%ld'", uxTaskGetStackHighWaterMark(NULL));
    ...
    buf[5933]= 0;
    httpd_resp_set_type(req, HTTPD_TYPE_TEXT);
    spi_flash_read(502 * 4096, buf, buf_len-1);
    httpd_resp_send(req, buf, buf_len);
    free(buf);
    return ESP_OK;
}

esp_err_t russia_get_handler(httpd_req_t *req)
{
    char*  buf;
    size_t buf_len;
    ESP_LOGI(TAG, "Free Stack for server task: '%ld'", uxTaskGetStackHighWaterMark(NULL));
    ...
    buf[6353]= 0;
    httpd_resp_set_type(req, HTTPD_TYPE_TEXT);
    spi_flash_read(504 * 4096, buf, buf_len-1);
    httpd_resp_send(req, buf, buf_len);
    free(buf);
    return ESP_OK;
}
...
httpd_uri_t basic_handlers[] = {
	    ...
	    { .uri      = "/js_heart.html",
	      .method   = HTTP_GET,
	      .handler  = heart_get_handler,
	      .user_ctx = NULL,
	    },
	    { .uri      = "/hello",
	      .method   = HTTP_GET,
	      .handler  = hello_get_handler,
	      .user_ctx = "hello,world,web test",
	    },
	    { .uri      = "/russia.html",
	      .method   = HTTP_GET,
	      .handler  = russia_get_handler,
	      .user_ctx = NULL,
	    },
	    { .uri      = "/echo",
	      .method   = HTTP_POST,
